...
use crate::game::{Game, HUD_HEIGHT, PLAY_HEIGHT, VIRTUAL_WIDTH};
use crate::gfx::SpriteManager;
use crate::input::InputManager;
use macroquad::prelude::*;

#[allow(dead_code)]
const VERSION: &str = "26.05.30.268";
...
enum AppState {
    ...
    Leaderboard {
        last_scores: Vec<(String, u32)>,
        all_scores: Vec<storage::ScoreEntry>,
    },
}
...
async fn main() {
    ...
    loop {
        ...
        let target_vheight = if input.touch_active {
            400.0
        } else {
        ...
        match state {
            ...
            AppState::GameOver => {
                if input.any_key {
                    state = AppState::EnteringName {
                        input: shared::input::TextInput::new(10, String::new()),
                    };
                }
            }
            AppState::EnteringName { ref mut input } => {
                ...
                if submitted {
                    let final_name = if input.content.is_empty() {
                        "BUB".to_string()
                    } else {
                        input.content.clone()
                    };
        ...
        match state {
            ...
            AppState::EnteringName { ref input } => {
                ...
                if is_mobile && cfg!(target_arch = "wasm32") {
                    ...
                    draw_text(
                        pop,
                        btn_x + (btn_w - pop_dims.width) / 2.0,
                        vy + 160.0 * scale,
                        12.0 * scale,
                        WHITE,
                    );
            ...
            AppState::Leaderboard {
                ...
                for (i, s) in all_scores.iter().take(10).enumerate() {
                    let is_highlight = last_scores
                        .iter()
                        .any(|(ln, ls)| ln == &s.name && ls == &s.score);
                ...
                draw_text(
                    footer,
                    center_x - f_dims.width / 2.0,
                    vy + target_vheight * scale - 15.0 * scale,
                    text_size,
                    GRAY,
                );
            }
